    ...
    member this.Loop() =
        let rec loop () =
            ...
            | "2" ->
                ...
                while exec.Read() do
                    let id = exec.GetInt32(0)
                    printfn "%d | %s | %d | %s" (id) (exec.GetString(1)) (exec.GetInt32(2)) (exec.GetString(3))    
                let id = int (InputHelper.ReadLine "ID filma: ")
                if existsFilm id conn then  
                    ...
                    cmd.ExecuteNonQuery() |> ignore
                    use auditCmd = conn.CreateCommand()
                    insertAudit $"ID={id} Film izmjenjen" auditCmd
                    auditCmd.ExecuteNonQuery() |> ignore
                    Console.WriteLine("Film izmjenjen!")
                else 
                    Console.WriteLine($"Film sa ID={id} ne postoji!")
            ...
            | "3" ->
                ...
                let id = int (InputHelper.ReadLine "ID filma: ")
                if existsFilm id conn then
                    if existsProjekcijaForFilm id conn then
                        Console.WriteLine("Film se ne moze obrisati jer ima zakazane projekcije!")
                    else
                        use cmd = conn.CreateCommand()
                        deleteFilm id cmd
                        cmd.ExecuteNonQuery() |> ignore
                        use auditCmd = conn.CreateCommand()
                        insertAudit $"{id} Film obrisan." auditCmd
                        auditCmd.ExecuteNonQuery() |> ignore
                        Console.WriteLine($"Film sa id: {id} je obrisan!")
                else
                    Console.WriteLine($"Film sa ID={id} ne postoji!")
                InputHelper.PressEnter()
                loop()
            ...
            | "4" ->
                ...
                if List.contains filmId idlist then 
                  use cmd = conn.CreateCommand()
                  insertProjekcija filmId salaId pocetak cijena cmd
                  cmd.ExecuteNonQuery() |> ignore
                  use auditCmd = conn.CreateCommand()
                  insertAudit $"Projekcija dodana za film ID={filmId}!" auditCmd
                  auditCmd.ExecuteNonQuery() |> ignore
                  Console.WriteLine("Projekcija dodana!")
                else 
                  Console.WriteLine($"Ne postoji film sa ID:{filmId}")
                InputHelper.PressEnter()
                loop()
            | "5" ->
                Console.WriteLine("Prikaz aktivnih projekcija: ")
                Console.WriteLine("Id | FilmId | SalaId | Pocetak | Cijena") 
                use conn = new SqliteConnection(connectionString)
                conn.Open()
                use cmd1 = conn.CreateCommand()
                cmd1.CommandText <- "Select * FROM Projekcija WHERE Aktivna = 1;"
                use exec = cmd1.ExecuteReader()
                while exec.Read() do
                    printfn "%d | %d | %d | %s | %.2f" (exec.GetInt32(0)) (exec.GetInt32(1)) (exec.GetInt32(2)) (exec.GetString(3)) (exec.GetDouble(4))    
                let id = int (InputHelper.ReadLine "ID projekcije: ")
                if existsProjekcija id conn then
                  let salaId = int (InputHelper.ReadLine "Nova sala (1 ili 2): ")
                  if salaId <> 1 && salaId <> 2 then 
                    Console.WriteLine("Nevalidan ID sale!")
                  else  
                  ...
                  Console.WriteLine($"Projekcija sa ID={id} ne postoji!")
                InputHelper.PressEnter()
                loop()
            | "6" ->
                Console.WriteLine("Prikaz aktivnih projekcija: ")
...
type User(connectionString: string) =
    member this.Loop() =
        let rec loop () =
            ...
            Console.WriteLine("3. Pretraga projekcija po nazivu filma i datumu")
            Console.WriteLine("4. Prikaz sale")
            Console.WriteLine("5. Kreiranje rezervacije")
            Console.WriteLine("6. Otkazivanje rezervacije")
            Console.WriteLine("7. Pretraga rezervacija po imenu korisnika")
            Console.WriteLine("0. Nazad")
            ...
            | "3" ->
                let datum = InputHelper.ReadLine "Datum (YYYY-MM-DD): "
                let naziv = InputHelper.ReadLine "Substring naziva filma: "

                use conn = new SqliteConnection(connectionString)
                conn.Open() 
                use cmd = conn.CreateCommand()
                selectProjekcijaByDateAndName datum naziv cmd
                use reader = cmd.ExecuteReader()
                Console.WriteLine("ID | Film | Sala | Pocetak | Cijena")
                while reader.Read() do
                    Console.WriteLine($"{reader.GetInt32(0)} | {reader.GetString(1)} | {reader.GetInt32(2)} | {reader.GetString(3)} | {reader.GetDouble(4)}")
            ...
            | "5" ->
                ...
                while exec.Read() do
                    printfn "%d | %d | %d | %s | %.2f" (exec.GetInt32(0)) (exec.GetInt32(1)) (exec.GetInt32(2)) (exec.GetString(3)) (exec.GetDouble(4))  
                let projId = int (InputHelper.ReadLine "ID projekcije: ")
                if existsProjekcija projId conn then 
                  ...
                  let id = Convert.ToInt32(cm.ExecuteScalar())
                  ...
                  Console.WriteLine($"Projekcija sa ID={projId} ne postoji!")
                InputHelper.PressEnter()
                loop()
            | "6" ->
                Console.WriteLine("Prikaz rezervacija: ")
                Console.WriteLine("Id | ProjekcijaId | BrojSjedista | ImeKorisnika ") 
                use conn = new SqliteConnection(connectionString)
                conn.Open()
                use cmd1 = conn.CreateCommand()
                cmd1.CommandText <- "Select * FROM Rezervacija;"
                use exec = cmd1.ExecuteReader()
                while exec.Read() do
                    printfn "%d | %d | %d | %s " (exec.GetInt32(0)) (exec.GetInt32(1)) (exec.GetInt32(2)) (exec.GetString(3))   
                let rezId = int (InputHelper.ReadLine "ID rezervacije: ")
                if existsRezervacija rezId conn then                  
                  use cmd = conn.CreateCommand()
                  deleteRezervacija rezId cmd
                  cmd.ExecuteNonQuery() |> ignore
                  use auditCmd = conn.CreateCommand()
                  insertAudit $"USER: Otkazana rezervacija ID={rezId}" auditCmd
                  auditCmd.ExecuteNonQuery() |> ignore
                  Console.WriteLine("Rezervacija otkazana!")
                else
                  Console.WriteLine($"Rezervacija sa ID={rezId} ne postoji")
                InputHelper.PressEnter()
                loop()
            | "7" ->
                let ime = InputHelper.ReadLine "Ime korisnika: "
